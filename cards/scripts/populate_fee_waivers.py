...
def populate_fee_waivers():
    for card_name, waiver_data in FEE_WAIVER_DATA.items():
        ...
        joining_fee = waiver_data.get("joining_fee")
        annual_fee = waiver_data.get("annual_fee")
        waiver_on_annual_spends = waiver_data.get("waiver_on_spend")

        # If the waiver_on_annual_spends is a dict (first_year/renewal), store as None or handle as needed
        if isinstance(waiver_on_annual_spends, dict):
            waiver_on_annual_spends = None  # Or handle differently if your model supports it
        ...
        fw, created = FeeWaiver.objects.update_or_create(
            ...
            defaults={
                ...
                "waiver_on_annual_spends": waiver_on_annual_spends,
            }
        )
        print(f"{'Created' if created else 'Updated'} FeeWaiver for {card_name}")
